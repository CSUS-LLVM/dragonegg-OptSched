...
ReadyList::ReadyList(DataDepGraph* dataDepGraph, SchedPriorities prirts) {
  ...
  useCntBits_ = crtclPathBits_ = scsrCntBits_ = ltncySumBits_ = nodeID_Bits_ = inptSchedOrderBits_ = 0;

  // Calculate the number of bits needed to hold the maximum value of each priority scheme
  for (i=0; i< prirts.cnt; i++) {
    switch (prirts.vctr[i]) {
      ...
      case LSH_LUC:
        ...
        keyedEntries_ = new KeyedEntry<SchedInstruction, unsigned long>*[dataDepGraph->GetInstCnt()];
      ...
      case LSH_ISO:
        ...
        inptSchedOrderBits_ = Utilities::clcltBitsNeededToHoldNum(maxInptSchedOrder_);
...
ReadyList::~ReadyList() {
  ...
  if (keyedEntries_) delete keyedEntries_;
}
...
unsigned long ReadyList::CmputKey_(SchedInstruction* inst, bool isUpdate, bool& changed) {
  unsigned long key = 0;
  int16_t keySize = 0;
  int i;
  int16_t oldLastUseCnt, newLastUseCnt;
  changed = true;
  if (isUpdate) changed = false;
  ...
  for (i=0; i < prirts_.cnt; i++) {
    switch (prirts_.vctr[i]) {
      ...
      case LSH_LUC:      
        ...
        if (newLastUseCnt != oldLastUseCnt) changed = true;
...
void ReadyList::AddInst(SchedInstruction* inst) {
  ...
  assert(changed == true);
  KeyedEntry<SchedInstruction, unsigned long>* entry = prirtyLst_->InsrtElmnt(inst, key, true);
...
void ReadyList::UpdatePriorities() {
  ...
  for (inst = prirtyLst_->GetFrstElmnt();
       inst != NULL;
