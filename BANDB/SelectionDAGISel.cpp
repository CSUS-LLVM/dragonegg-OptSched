...
//    std::cout << "Calling OptSched\n";

	if (OptLevel == CodeGenOpt::None || ST.enableMachineScheduler() ||
		TLI.getSchedulingPreference() == Sched::Source)
	  return createSourceListDAGScheduler(IS, OptLevel);
	if (TLI.getSchedulingPreference() == Sched::RegPressure)
	   return createOptDAGScheduler(IS, OptLevel);
	   // NOTE: Original LLVM uses the BURR List scheduler in this case.
	...
	return createOptDAGScheduler(IS, OptLevel);
     ...
     // NOTE: Original LLVM uses the ILP scheduler in this case.
