...
namespace Stats {
  ...
  template<class T>
  void DistributionStat<T>::Record(T value) {
    count_++;
    sum_ += value;
  ...
  template<class T>
  void DistributionStat<T>::Print(std::ostream& out) const {
  ...
  template<class T>
  void SetStat<T>::Print(std::ostream& out) const {
    out << name_ << ": ";
    if (values_.size() == 0) {
      out << "[no records]";
    } else {
  ...
  template<class T>
  void IndexedSetStat<T>::Print(std::ostream& out) const {
    out << name_ << ":";
    if (values_.size() == 0) {
      out << " [no records]\n";
    } else {
  ...
  void TimeoutStat::Record(int regionNumber,
                           InstCount instCount,
                           int lowerBound,
                           int upperBound) {
  ...
  void TimeoutStat::Print(ostream& out) const {
    ...
    for (std::list<Entry>::const_iterator it = entries_.begin();
         it != entries_.end();
         it++) {
      out << "  Region #" << it->regionNumber
          << " (" << it->instCount << " instructions):"
          << " final range = [" << it->lowerBound
          << ", " << it->upperBound << "].\n";
    }
  }

  template<class T>
  void IndexedNumericStat<T>::Print(std::ostream& out) const {
    out << std::setprecision(4);
    out << name_ << ":";
    if (values_.size() == 0) {
      out << " [no records]";
  ...
  IntDistributionStat solutionTimeForSolvedProblems(
      "Solution time for solved problems");
  ...
  IntDistributionStat feasibleSchedulesPerLength(
      "Feasible schedules per length");
  ...
  IntDistributionStat dynamicLBIterationsPerPath(
      "Dynamic LB iterations per path");
  ...
  IntDistributionStat traceHeuristicScheduleLength(
      "Trace heuristic schedule length");
  IntDistributionStat traceOptimalScheduleLength(
      "Trace optimal schedule length");
  ...
  IntDistributionStat enumerationToHeuristicTimeRatio(
      "Enumeration to heuristic time ratio");
  IntDistributionStat verificationTime("Verification time");

  IntDistributionStat historyEntriesPerIteration(
  ...
  IntDistributionStat historyTableInitializationTime(
      "History table initialization time");
  ...
  IntStat positiveUpperBoundMismatchCount(
      "Positive upper bound mismatch count");
  IntStat negativeUpperBoundMismatchCount(
      "Negative upper bound mismatch count");
  IntStat positiveLowerBoundMismatchCount(
      "Positive lower bound mismatch count");
  IntStat negativeLowerBoundMismatchCount(
      "Negative lower bound mismatch count");
  ...
  IntStat nodeSuperiorityInfeasibilityHits(
      "Node superiority infeasibility hits");
  IntStat rangeTighteningInfeasibilityHits(
      "Range tightening infeasibility hits");
  IntStat historyDominationInfeasibilityHits(
      "History domination infeasibility hits");
  IntStat relaxedSchedulingInfeasibilityHits(
      "Relaxed scheduling infeasibility hits");
  IntStat slotCountInfeasibilityHits(
      "Slot count infeasibility hits");
  IntStat forwardLBInfeasibilityHits(
      "Forward LB infeasibility hits");
  IntStat backwardLBInfeasibilityHits(
      "Backward LB infeasibility hits");

  IntStat invalidSchedules("Invalid schedules");

  IntStat totalInstructions("Total instructions");
  IntStat instructionsWithTighterFileLB(
      "Instructions with tighter file LB");
  IntStat cyclesTightenedForTighterFileLB(
      "Cycles tightened for instructions with tighter file LB");
  IntStat instructionsWithTighterRelaxedLB(
      "Instructions with tighter relaxed LB");
  IntStat cyclesTightenedForTighterRelaxedLB(
      "Cycles tightened for instructions with tighter relaxed LB");
  IntStat instructionsWithEqualLB(
      "Instructions with equal LB");
  IntStat instructionsWithTighterFileUB(
      "Instructions with tighter file UB");
  IntStat cyclesTightenedForTighterFileUB(
      "Cycles tightened for instructions with tighter file UB");
  IntStat instructionsWithTighterRelaxedUB(
      "Instructions with tighter relaxed UB");
  IntStat cyclesTightenedForTighterRelaxedUB(
      "Cycles tightened for instructions with tighter relaxed UB");
  IntStat instructionsWithEqualUB(
      "Instructions with equal UB");
  ...
  IntStat legalListSchedulerInstructionHits(
      "Legal list scheduler instruction hits");
