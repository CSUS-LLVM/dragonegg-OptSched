...
SchedInstruction::SchedInstruction(InstCount num,
                                   const string& name,
                                   InstType instType,
                                   const string& opCode,
  ...
  if (crntRange_ == NULL) Logger::Fatal("Out of memory.");
...
bool SchedInstruction::UseFileBounds() {
  bool match = true;
    ...
    if (frwrdLwrBound_ > fileLwrBound_) {
      ...
      Stats::cyclesTightenedForTighterRelaxedLB +=
          frwrdLwrBound_ - fileLwrBound_;
    }
    ...
    if (bkwrdLwrBound_ > fileUprBound_) {
      ...
      Stats::cyclesTightenedForTighterRelaxedUB +=
          bkwrdLwrBound_ - fileUprBound_;
    }
...
bool SchedInstruction::InitForSchdulng(InstCount schedLngth,
  ...
  if (schedLngth != INVALID_VALUE) {
    bool fsbl = crntRange_->SetBounds(
        frwrdLwrBound_, bkwrdLwrBound_, schedLngth, fxdLst);
...
void SchedInstruction::AllocMem_(InstCount instCnt,
                                 bool isCP_FromScsr,
                                 bool isCP_FromPrdcsr) {
  ...
  for (GraphEdge* edge = prdcsrLst_->GetFrstElmnt();
       edge != NULL;
       edge = prdcsrLst_->GetNxtElmnt()) {
    ltncyPerPrdcsr_[predecessorIndex++] = edge->label;
    sortedPrdcsrLst_->InsrtElmnt(
        (SchedInstruction*)edge->GetOtherNode(this), edge->label, true);
  }

  if (isCP_FromScsr) {
    crtclPathFrmRcrsvScsr_ = new InstCount[instCnt];
    if (crtclPathFrmRcrsvScsr_ == NULL) Logger::Fatal("Out of memory.");
  ...
  if (isCP_FromPrdcsr) {
    ...
    if (crtclPathFrmRcrsvPrdcsr_ == NULL) Logger::Fatal("Out of memory.");
...
void SchedInstruction::DeAllocMem_() {
  ...
  if (rdyCyclePerPrdcsr_ != NULL) delete[] rdyCyclePerPrdcsr_;
  if (prevMinRdyCyclePerPrdcsr_ != NULL) delete[] prevMinRdyCyclePerPrdcsr_;
  if (ltncyPerPrdcsr_ != NULL) delete[] ltncyPerPrdcsr_;
  if (sortedPrdcsrLst_ != NULL) delete sortedPrdcsrLst_;
  if (sortedScsrLst_ != NULL) delete sortedScsrLst_;
  if (crtclPathFrmRcrsvScsr_ != NULL) delete[] crtclPathFrmRcrsvScsr_;
  if (crtclPathFrmRcrsvPrdcsr_ != NULL) delete[] crtclPathFrmRcrsvPrdcsr_;
...
InstCount SchedInstruction::CmputCrtclPath_(DIRECTION dir,
  ...
  for (GraphEdge* edg = nghbrLst->GetFrstElmnt();
       edg != NULL;
    ...
    if (ref == NULL) {
      ...
      if (!ref->IsRcrsvNghbr(dir, nghbr)) continue;
...
//num_, reg->GetNum(), reg->GetType(), reg->GetPhysicalNumber(), reg->GetUseCnt());
  assert(reg != NULL);
  defs_[defCnt_++] = reg;
}
...
bool SchedInstruction::BlocksCycle() const {
  return blksCycle_;
}

bool SchedInstruction::IsPipelined() const {
  return pipelined_;
}
...
void SchedInstruction::SetMustBeInBBEntry(bool val) {
    mustBeInBBEntry_ = val;
}

void SchedInstruction::SetMustBeInBBExit(bool val) {
    mustBeInBBExit_ = val;
}

const char * SchedInstruction::GetName() const {
  return name_.c_str();
}

const char * SchedInstruction::GetOpCode() const {
  return opCode_.c_str();
}

int SchedInstruction::GetNodeID() const {
  return nodeID_;
}
...
SchedInstruction* SchedInstruction::GetFrstPrdcsr(InstCount* scsrNum,
  ...
  if (!edge) return NULL;
  if (scsrNum) *scsrNum = edge->succOrder;
  if (ltncy) *ltncy = edge->label;
  if (depType) *depType = (DependenceType)edge->label2;
...
SchedInstruction* SchedInstruction::GetFrstScsr(InstCount* prdcsrNum,
  ...
  if (!edge) return NULL;
  if (prdcsrNum) *prdcsrNum = edge->predOrder;
  if (ltncy) *ltncy = edge->label;
  if (depType) *depType = (DependenceType)edge->label2;
...
InstCount SchedInstruction::CmputCrtclPathFrmRcrsvPrdcsr(
    SchedInstruction* ref) {
...
void SchedInstruction::SetLwrBound(DIRECTION dir,
                                   InstCount bound,
                                   bool isAbslut) {
  if (dir == DIR_FRWRD) {
...
bool SchedInstruction::PrdcsrSchduld(InstCount prdcsrNum,
                                     InstCount cycle,
                                     InstCount& rdyCycle) {
...
void SchedInstruction::SetInstType(InstType type) {
  instType_ = type;
}

void SchedInstruction::SetIssueType(IssueType type) {
  issuType_ = type;
}

InstType SchedInstruction::GetInstType() const {
  return instType_;
}

IssueType SchedInstruction::GetIssueType() const {
  return issuType_;
}

bool SchedInstruction::IsSchduld(InstCount* cycle) const {
  if (cycle) *cycle = crntSchedCycle_;
...
void SchedInstruction::SetRlxdCycle(InstCount cycle) {
  crntRlxdCycle_ = cycle;
}
...
void SchedInstruction::SetSig(InstSignature sig) {
  sig_ = sig;
}

InstSignature SchedInstruction::GetSig() const {
  return sig_;
}
...
bool SchedInstruction::IsFxd() const {
  return crntRange_->IsFxd();
}

InstCount SchedInstruction::GetPreFxdCycle() const {
  return preFxdCycle_;
}

bool SchedInstruction::TightnLwrBound(DIRECTION dir,
                                      InstCount newLwrBound,
                                      LinkedList<SchedInstruction>* tightndLst,
                                      LinkedList<SchedInstruction>* fxdLst,
                                      bool enforce) {
  return crntRange_->TightnLwrBound(
      dir, newLwrBound, tightndLst, fxdLst, enforce);
}
...
bool SchedInstruction::TightnLwrBoundRcrsvly(
    ...
    LinkedList<SchedInstruction>* fxdLst,
    bool enforce) {
  return crntRange_->TightnLwrBoundRcrsvly(
      dir, newLwrBound, tightndLst, fxdLst, enforce);
}

bool SchedInstruction::ProbeScsrsCrntLwrBounds(InstCount cycle) {
  if (cycle <= crntRange_->GetLwrBound(DIR_FRWRD)) return false;
  ...
  for (GraphEdge* edg = nghbrLst->GetFrstElmnt();
    ...
    // given cycle.
    if (nghbrNewLwrBound > nghbr->GetCrntLwrBound(DIR_FRWRD)) return true;
  }
...
void SchedInstruction::SetScsrNums_() {
  ...
  for (GraphEdge* edge = scsrLst_->GetFrstElmnt();
       edge != NULL;
...
bool SchedRange::TightnLwrBound(DIRECTION dir,
                                InstCount newBound,
                                LinkedList<SchedInstruction>* tightndLst,
                                LinkedList<SchedInstruction>* fxdLst,
                                bool enforce) {
  ...
  InstCount* prevBoundPtr = (dir == DIR_FRWRD) ? &prevFrwrdLwrBound_
                                               : &prevBkwrdLwrBound_;
...
bool SchedRange::TightnLwrBoundRcrsvly(DIRECTION dir,
                                       InstCount newBound,
                                       LinkedList<SchedInstruction>* tightndLst,
                                       LinkedList<SchedInstruction>* fxdLst,
                                       bool enforce) {
  LinkedList<GraphEdge>* nghbrLst = (dir == DIR_FRWRD) ? inst_->scsrLst_
                                                       : inst_->prdcsrLst_;
...
bool SchedRange::Fix(InstCount cycle,
                     LinkedList<SchedInstruction>* tightndLst,
                     LinkedList<SchedInstruction>* fxdLst) {
  if (cycle < frwrdLwrBound_ || cycle > GetDeadline()) return false;
...
void SchedRange::SetBounds(InstCount frwrdLwrBound,
                           InstCount bkwrdLwrBound) {
  InitVars_();
  frwrdLwrBound_ = frwrdLwrBound;
  bkwrdLwrBound_ = bkwrdLwrBound;
}

bool SchedRange::SetBounds(InstCount frwrdLwrBound,
                           InstCount bkwrdLwrBound,
...
bool SchedRange::IsFsbl_() const {
  return GetLwrBoundSum_() <= lastCycle_;
}
...
bool SchedRange::IsFxd() const {
  return lastCycle_ == GetLwrBoundSum_();
}
...
void SchedRange::SetLwrBound(DIRECTION dir, InstCount bound) {
  ...
  crntBound = bound;
  #ifdef IS_DEBUG
    InstCount crntBoundPtr = (dir == DIR_FRWRD) ? frwrdLwrBound_
                                                : bkwrdLwrBound_;
