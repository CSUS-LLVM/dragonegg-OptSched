...
EnumTreeNode::~EnumTreeNode() {
  ...
  if (isCnstrctd_) {
    ...
    if (rdyLst_ != NULL) delete rdyLst_;
...
void EnumTreeNode::Construct(EnumTreeNode* prevNode, SchedInstruction* inst,
  ...
  if (isCnstrctd_ == false) {
    ...
    if (exmndInsts_ == NULL) Logger::Fatal("Out of memory.");

    chldrn_ = new LinkedList<HistEnumTreeNode>(instCnt);
    if (chldrn_ == NULL) Logger::Fatal("Out of memory.");

    frwrdLwrBounds_ = new InstCount[instCnt];
    if (frwrdLwrBounds_ == NULL) Logger::Fatal("Out of memory.");
  }
...
void EnumTreeNode::SetLwrBounds() {
  SetLwrBounds(DIR_FRWRD);
}
/*****************************************************************************/
      ...
      if (enumrtr_->prune_.nodeSup) {
        if (!isNodeDmntd) {
          ...
          exmndInst = new ExaminedInst(inst, wasRlxInfsbl,
                                       enumrtr_->dirctTightndLst_);
          if (exmndInst == NULL) Logger::Fatal("Out of memory.");
...
bool EnumTreeNode::WasSprirNodeExmnd(SchedInstruction* cnddtInst) {
  if (cnddtInst == NULL) return false;
...
bool EnumTreeNode::WasRsrcDmnntNodeExmnd(SchedInstruction* cnddtInst) {
  ...
  for (exmndInst = exmndInsts_->GetFrstElmnt();
       exmndInst != NULL;
       exmndInst = exmndInsts_->GetNxtElmnt()) {
...
//contradiction that the given instruction is infeasible for this slot
  ...
  if (exmndInst == NULL) return false;

  SchedInstruction* inst = exmndInst->GetInst();
  assert(inst->IsSchduld() == false);

  if (cnddtInst->GetIssueType() != inst->GetIssueType()) return false;
  ...
  if (cnddtInst->ProbeScsrsCrntLwrBounds(deadline)) return false;
...
EnumTreeNode::ExaminedInst::~ExaminedInst() {
  if (tightndScsrs_ != NULL) {
    for (TightndInst* inst = tightndScsrs_->GetFrstElmnt();
         inst != NULL;
...
Enumerator::Enumerator(DataDepGraph* dataDepGraph,
  ...
  if (rlxdSchdulr_ == NULL) Logger::Fatal("Out of memory.");
  ...
  if (IsHistDom()) {
    exmndSubProbs_ = new BinHashTable<HistEnumTreeNode>(sigSize, sigHashSize, true);
    if (exmndSubProbs_ == NULL) Logger::Fatal("Out of memory.");
  }
  ...
  if (tightndLst_ == NULL) Logger::Fatal("Out of memory.");

  fxdLst_ = new LinkedList<SchedInstruction>(totInstCnt_);
  if (fxdLst_ == NULL) Logger::Fatal("Out of memory.");

  dirctTightndLst_ = new LinkedList<SchedInstruction>(totInstCnt_);
  if (dirctTightndLst_ == NULL) Logger::Fatal("Out of memory.");

  bkwrdTightndLst_ = new LinkedList<SchedInstruction>(totInstCnt_);
  if (bkwrdTightndLst_ == NULL) Logger::Fatal("Out of memory.");

  tmpLwrBounds_ = new InstCount[totInstCnt_];
  if (tmpLwrBounds_ == NULL) Logger::Fatal("Out of memory.");
...
void Enumerator::SetupAllocators_() {
  ...
  if (nodeAlctr_ == NULL) Logger::Fatal("Out of memory.");

  if (IsHistDom()) {
    hashTblEntryAlctr_ = new MemAlloc<BinHashTblEntry<HistEnumTreeNode> >(memAllocBlkSize);
    if (hashTblEntryAlctr_ == NULL) Logger::Fatal("Out of memory.");
    ...
    if (lastInsts_ == NULL) Logger::Fatal("Out of memory.");

    othrLastInsts_ = new SchedInstruction*[totInstCnt_];

    if (othrLastInsts_ == NULL) Logger::Fatal("Out of memory.");
  }
}
/****************************************************************************/
...
void Enumerator::ResetAllocators_() {
  ...
  if (IsHistDom()) hashTblEntryAlctr_->Reset();
}
/****************************************************************************/
...
FUNC_RESULT Enumerator::FindFeasibleSchedule_(InstSchedule *sched,
  ...
  while (!(allNodesExplrd || WasObjctvMet_())) {
    ...
    if (foundFsblBrnch) {
      ...
      auto matchingHistNodesWithSuffix = FindMatchingHistNodes(nxtNode, IsHistDom(), exmndSubProbs_, this);
  ...
  if (isTimeout) return RES_TIMEOUT;
...
bool Enumerator::FindNxtFsblBrnch_(EnumTreeNode*& newNode) {
  ...
  #endif
  assert(crntNode_->IsLeaf() ||
         (brnchCnt != rdyInstCnt) ? 1 : rdyInstCnt);
  ...
  for (i = crntBrnchNum; i < brnchCnt && crntNode_->IsFeasible(); i++) {
    ...
    if (i == brnchCnt - 1) {
      ...
      if (isEmptyNode || crntNode_->GetLegalInstCnt() == 0 ||
          enumStall) {
        #ifdef IS_DEBUG_INFSBLTY_TESTS
          Stats::stalls++;
        #endif
      } else {
        crntNode_->NewBranchExmnd(inst, false, false, false, false,
                                  DIR_FRWRD, false);
        continue;
      }
    } else {
    ...
    if (ProbeBranch_(inst, newNode, isNodeDmntd, isRlxInfsbl, isLngthFsbl)) {
      ...
      crntNode_->NewBranchExmnd(inst, true, isNodeDmntd, isRlxInfsbl,
                                false, DIR_FRWRD, isLngthFsbl);
    }
  }
...
bool Enumerator::ProbeBranch_(SchedInstruction* inst, EnumTreeNode*& newNode,
  ...
  Logger::Info("Probing inst %d in cycle %d / slot %d",
               instNum, crntCycleNum_, crntSlotNum_);
#endif
...
void Enumerator::RestoreCrntState_(SchedInstruction* inst, EnumTreeNode* newNode) {
  if (newNode != NULL) {
...
void Enumerator::StepFrwrd_(EnumTreeNode*& newNode) {
  ...
  Logger::Info("Stepping forward from node %lld to node %lld by scheduling "
               ...
               crntNode_->GetParent()->GetNum(),
               crntNode_->GetNum(),
               instNumToSchdul,
               crntCycleNum_,
               crntNode_->GetCostLwrBound());
#endif
...
bool Enumerator::TightnLwrBounds_(SchedInstruction* newInst) {
  ...
  for (i = 0; i < issuTypeCnt_; i++) {
    ...
    if (newInst == NULL) {
      ...
      //next cycle
      nxtAvlblCycle[i] = avlblSlotsInCrntCycle_[i] == 0 ?
                         crntCycleNum_ + 1 : crntCycleNum_;
    }
  }

  for (i = minUnschduldTplgclOrdr_; i < totInstCnt_; i++) {
    inst = dataDepGraph_->GetInstByTplgclOrdr(i);
    assert(inst != newInst || inst->GetCrntLwrBound(DIR_FRWRD) == crntCycleNum_);
...
#ifdef IS_DEBUG_FLOW
        Logger::Info("Tightening LB of inst %d from %d to %d",
                     inst->GetNum(),
                     inst->GetCrntLwrBound(DIR_FRWRD),
                     newLwrBound);
#endif
        fsbl = inst->TightnLwrBoundRcrsvly(DIR_FRWRD, newLwrBound,
                                           tightndLst_, fxdLst_, false);
  ...
  for (inst = tightndLst_->GetFrstElmnt();
       inst != NULL;
...
bool Enumerator::FixInsts_(SchedInstruction* newInst) {
  ...
  for (SchedInstruction* inst = fxdLst_->GetFrstElmnt();
       inst != NULL;
    ...
    if (fsbl == false) {
#ifdef IS_DEBUG_FLOW
      Logger::Info("Can't fix inst %d in cycle %d",
                   inst->GetNum(), inst->GetFxdCycle());
#endif
      break;
    }
...
bool Enumerator::RlxdSchdul_(EnumTreeNode* newNode) {
  ...
  if (rsrcFxdLst == NULL) Logger::Fatal("Out of memory.");

  bool fsbl = rlxdSchdulr_->SchdulAndChkFsblty(crntCycleNum_, trgtSchedLngth_ - 1);

  for (SchedInstruction* inst = rsrcFxdLst->GetFrstElmnt();
       inst != NULL;
...
bool Enumerator::EnumStall_() {
  return enblStallEnum_;
}
/*****************************************************************************/
...
LengthEnumerator::LengthEnumerator(DataDepGraph* dataDepGraph,
  ...
  if (tmpHstryNode_ == NULL) Logger::Fatal("Out of memory.");
}
/*****************************************************************************/
...
void LengthEnumerator::SetupAllocators_() {
  ...
  if (IsHistDom()) {
    ...
    if (histNodeAlctr_ == NULL) Logger::Fatal("Out of memory.");
  }
}
/****************************************************************************/
...
void LengthEnumerator::ResetAllocators_() {
  ...
  if (IsHistDom()) histNodeAlctr_->Reset();
}
/****************************************************************************/
...
bool LengthEnumerator::IsCostEnum() {
  return false;
}
/*****************************************************************************/
...
LengthCostEnumerator::LengthCostEnumerator(DataDepGraph* dataDepGraph,
                                           MachineModel* machMdl,
                                           InstCount schedUprBound,
                                           int16_t sigHashSize,
                                           SchedPriorities prirts,
...
bool LengthCostEnumerator::IsCostEnum() {
  return true;
}
/*****************************************************************************/

void LengthCostEnumerator::Reset() {
  Enumerator::Reset();
}
/*****************************************************************************/
...
bool LengthCostEnumerator::ChkCostFsblty_(SchedInstruction* inst, EnumTreeNode*& newNode) {
  bool isFsbl = true;
...
#endif
      rgn_->UnschdulInst(inst, crntCycleNum_, crntSlotNum_, newNode->GetParent());
    }
  }
...
InstCount LengthCostEnumerator::GetBestCost_() {
  return rgn_->GetBestCost();
}
/*****************************************************************************/
...
//Logger::Info("enblStallEnum_ = %d", enblStallEnum_); 
  if (!enblStallEnum_) return false;
  if (crntNode_->IsNxtSlotStall()) return true;
  if (crntNode_ == rootNode_) return false;
  if (dataDepGraph_->IncludesUnpipelined()) return true;
//  return false;
  return true;
}
/*****************************************************************************/
