...
template<class T>
class ArrayMemAlloc: public MemAlloc<T> {
  public:
    ...
    inline T* GetArray() {
      return MemAlloc<T>::GetObjects_(arraySize_);
    }
    // Frees an array of objects and recycle it for future use.
    inline void FreeArray(T* array) {
      FreeObject(array);
    }
...
template<class T>
inline MemAlloc<T>::~MemAlloc() {
  for (T* blk = allocatedBlocks_.GetFrstElmnt();
       blk != NULL;
...
template<class T>
inline void MemAlloc<T>::GetNewBlock_() {
  currentBlock_ = NULL;
...
template<class T>
inline void MemAlloc<T>::AllocNewBlock_() {
  T* blk = new T[blockSize_];
  if (blk == NULL) Logger::Fatal("Out of memory.");
...
template<class T>
inline T* MemAlloc<T>::GetObjects_(int count) {
...
template<class T>
inline T* MemAlloc<T>::GetObject() {
  return GetObjects_(1);
}

template<class T>
inline void MemAlloc<T>::FreeObject(T* obj) {
