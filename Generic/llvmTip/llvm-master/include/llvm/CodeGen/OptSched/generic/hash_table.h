...
template <class T>
class BinHashTblEntry : public HashTblEntry<T> {
  public:
...
template <class T>
class StrHashTblEntry : public HashTblEntry<T> {
  public:
...
class HashTable {
  public:
    ...
    UDT_HASHTBL_CPCTY GetEntryCnt() {
      return entryCnt_;
    }
    UDT_HASHTBL_CPCTY GetPpultdBktCnt() {
      return ppultdBktCnt_;
    }
    UDT_HASHTBL_CPCTY GetMaxListSize() {
      return maxListSize_;
    }
    ...
    virtual void Clear(
        bool del, MemAlloc<BinHashTblEntry<T> >* entryAlctr = NULL);
...
template<class T>
class BinHashTable: public HashTable<T> {
  public:
...
//----------------------------------------------------------------------
    ...
    bool DeleteEntry(UDT_HASHKEY key,
                     T* elmnt,
                     MemAlloc<HashTblEntry<T> >* entryAlctr = NULL);
    bool DeleteEntry(UDT_HASHVAL hashVal,
                     T* elmnt,
                     MemAlloc<HashTblEntry<T> >* entryAlctr = NULL);
...
template<class T>
class StrHashTable: public HashTable<T> {
  public:
...
template<class T>
inline void HashTblEntry<T>::Init_() {
  nxt_ = NULL;
  prev_ = NULL;
}
...
template<class T>
HashTblEntry<T>* HashTblEntry<T>::GetNxt() const {
  return nxt_;
}

template<class T>
inline void HashTblEntry<T>::SetNxt(HashTblEntry* newEntry) {
  nxt_ = newEntry;
}

template<class T>
HashTblEntry<T>* HashTblEntry<T>::GetPrev() const {
    return prev_;
}
...
template<class T>
inline T* HashTblEntry<T>::GetElmnt() const {
  return elmnt_;
}

template<class T>
inline BinHashTblEntry<T>::BinHashTblEntry() {
  Init_();
}

template<class T>
inline BinHashTblEntry<T>::BinHashTblEntry(
    UDT_HASHKEY key, T* elmnt, UDT_HASHVAL hashVal) {
  Construct(key, elmnt, hashVal);
}

template<class T>
inline void BinHashTblEntry<T>::Init_() {
  HashTblEntry<T>::Init_();
  key_ = 0;
}

template<class T>
inline void BinHashTblEntry<T>::Construct(
    UDT_HASHKEY key, T* elmnt, UDT_HASHVAL hashVal) {
  Init_();
...
template<class T>
inline UDT_HASHKEY BinHashTblEntry<T>::GetKey() {
  return key_;
}

template<class T>
inline UDT_HASHVAL BinHashTblEntry<T>::GetHashVal() {
  return HashTblEntry<T>::hashVal_;
}

template<class T>
inline StrHashTblEntry<T>::StrHashTblEntry(
    const char* name, T* elmnt, UDT_HASHVAL hashVal, UDT_HASHTBL_CPCTY indx) {
...
template<class T>
inline void StrHashTblEntry<T>::Init_() {
  name_ = NULL;
  indx_ = 0;
}

template<class T>
inline void StrHashTblEntry<T>::Construct(
    const char* name, T* elmnt, UDT_HASHVAL hashVal, UDT_HASHTBL_CPCTY indx) {
  Init_();
  HashTblEntry<T>::Construct_(elmnt, hashVal);
  name_ = new char[strlen(name)+1];

  if (name_ == NULL) Logger::Fatal("Out of memory.");
...
template<class T>
inline void StrHashTblEntry<T>::Clean() {
  if (name_ != NULL) delete[] name_;
}

template<class T>
inline UDT_HASHVAL StrHashTblEntry<T>::GetHashVal() {
  return HashTblEntry<T>::hashVal_;
}
...
template<class T>
inline T* StrHashTblEntry<T>::GetElmnt(
    const char* srchName, UDT_HASHTBL_CPCTY& indx) {
  indx = indx_;
...
template<class T>
inline UDT_HASHTBL_CPCTY StrHashTblEntry<T>::GetIndx() {
  return indx_;
}
...
template<class T>
inline bool HashTable<T>::IsConstructed() {
  return isCnstrctd_;
}
...
HashTable<T>::~HashTable() {
  if (isCnstrctd_) Clear(false);
  if (topEntry_) delete[] topEntry_;
  if (lastEntry_) delete[] lastEntry_;
  if (entryCnts_) delete[] entryCnts_;
}

template<class T>
void HashTable<T>::AddNewEntry_(
    HashTblEntry<T>* newEntry, UDT_HASHVAL hashVal) {
...
template<class T>
void HashTable<T>::FindNewMaxHash_() {
...
UDT_HASHVAL HashTable<T>::FindNextHash_(UDT_HASHVAL crntHash) {
  if (entryCnt_ == 0) return 0;

  assert(crntHash > 0);

  for (UDT_HASHVAL nxtHash = crntHash - 1; nxtHash != 0; nxtHash--) {
    if (entryCnts_[nxtHash] > 0) return nxtHash;
  }
...
template<class T>
void HashTable<T>::GetFullList(LinkedList<T>* lst) {
  for (UDT_HASHVAL crntHash = maxHash_;
       crntHash >= 0 && crntHash <= maxHash_;
       crntHash--) {
    if (entryCnts_[crntHash] > 0) {
      for (HashTblEntry<T>* crntEntry = topEntry_[crntHash];
           crntEntry != NULL;
...
template<class T>
BinHashTable<T>::BinHashTable(int16_t keyBitCnt,
                              int16_t hashBitCnt,
                              bool isExtrnlAlctr,
...
template<class T>
void BinHashTable<T>::Clear(
    bool del, MemAlloc<BinHashTblEntry<T> >* entryAlctr) {
  HashTable<T>::Clear(del, entryAlctr);
  Init_();
}

template<class T>
void BinHashTable<T>::Init_() {
  srchPtr_ = NULL;
  srchKey_ = 0;
...
template<class T>
inline void BinHashTable<T>::CmputConsts_() {
...
inline UDT_HASHVAL BinHashTable<T>::HashKey(UDT_HASHKEY key) {
  if (keyBitCnt_ == hashBitCnt_) return (UDT_HASHVAL)key;
  return ((UDT_HASHVAL)key >> hashRShft_);
}

template<class T>
HashTblEntry<T>* BinHashTable<T>::InsertElement(
    const UDT_HASHKEY key, T* elmnt, MemAlloc<BinHashTblEntry<T> >* allocator) {
  if (this->entryCnt_ == this->maxEntryCnt_) return NULL;
  ...
  if (this->isExtrnlAlctr_) {
    ...
    if (newEntry == NULL) Logger::Fatal("Out of memory.");
  }

  HashTable<T>::AddNewEntry_(newEntry, hashVal);

  if (key > maxKey_) maxKey_ = key;
...
template<class T>
void BinHashTable<T>::ReInsertEntry(HashTblEntry<T>* entry) {
  UDT_HASHVAL hashVal = entry->GetHashVal();
...
template<class T>
T* BinHashTable<T>::ExtractMax() {
  T* elmnt;
  HashTblEntry<T>* maxEntry;

  if (this->entryCnt_ == 0) return NULL;
...
template<class T>
HashTblEntry<T>* BinHashTable<T>::GetMaxEntry() {
  if (this->entryCnt_ == 0) {
    return NULL;
  }
...
template<class T>
void BinHashTable<T>::UpdtLastMax_(
    HashTblEntry<T>* maxEntry, UDT_HASHVAL maxHash) {
  if (lastMaxEntry_ != NULL) delete lastMaxEntry_;
  lastMaxEntry_ = maxEntry;
  lastMaxHash_ = maxHash;
}

template<class T>
void BinHashTable<T>::RestoreLastMax() {
...
T* BinHashTable<T>::GetFirstMatch(const UDT_HASHKEY key, bool skipCollision) {
  ...
  if (skipCollision) FindNextMatch_();

  return srchPtr_ == NULL ? NULL : srchPtr_->GetElmnt();
}

template<class T>
T* BinHashTable<T>::GetNextMatch(bool skipCollision) {
...
template<class T>
void BinHashTable<T>::FindNextMatch_() {
  for (; srchPtr_ != NULL; srchPtr_ = srchPtr_->GetNxt()) {
    if (((BinHashTblEntry<T>*)srchPtr_)->GetKey() == srchKey_) return;
  }
}
...
T* BinHashTable<T>::GetLastMatch(const UDT_HASHKEY key, bool skipCollision) {
  ...
  if (skipCollision) FindPrevMatch_();

  return srchPtr_ == NULL ? NULL : srchPtr_->GetElmnt();
}

template<class T>
T* BinHashTable<T>::GetPrevMatch(bool skipCollision) {
  assert(srchPtr_ != NULL);
...
template<class T>
void BinHashTable<T>::FindPrevMatch_() {
...
void BinHashTable<T>::DeleteEntries(UDT_HASHKEY key, LinkedList<T>* elmntLst) {
  ...
  for (T* elmnt = elmntLst->GetFrstElmnt();
       elmnt != NULL;
...
template<class T>
bool BinHashTable<T>::DeleteEntry(
    UDT_HASHVAL hashVal, T* elmnt, MemAlloc<HashTblEntry<T> >* entryAlctr) {
  if (hashVal < 0 || hashVal > this->maxHash_) return false;

  for (HashTblEntry<T>* entry = this->topEntry_[hashVal];
       entry != NULL;
       entry = entry->GetNxt()) {
...
template<class T>
bool BinHashTable<T>::DeleteEntry(
    UDT_HASHKEY key, T* elmnt, MemAlloc<HashTblEntry<T> >* entryAlctr) {
  UDT_HASHVAL hashVal = this->HashKey_(key);
  return DeleteEntry(hashVal, elmnt, entryAlctr);
}

template<class T>
StrHashTable<T>::StrHashTable(
    UDT_HASHVAL size, bool useIndx, UDT_HASHTBL_CPCTY maxEntryCnt)
...
template<class T>
StrHashTable<T>::~StrHashTable() {
  if (indxdTbl_) delete[] indxdTbl_;
}

template<class T>
UDT_HASHVAL StrHashTable<T>::HashString_(const char* strng) {
  int i = 0;
...
template<class T>
T* StrHashTable<T>::FindElement(const char* srchName) {
  UDT_HASHVAL hashVal = HashString_(srchName);
  T* elmnt;

  for (HashTblEntry<T>* crnt = this->topEntry_[hashVal];
       crnt != NULL;
       crnt = crnt->GetNxt()) {
...
FUNC_RESULT StrHashTable<T>::InsertElement(const char* name, T* elmnt) {
  ...
  if (this->entryCnt_ == this->maxEntryCnt_) return RES_FAIL;
