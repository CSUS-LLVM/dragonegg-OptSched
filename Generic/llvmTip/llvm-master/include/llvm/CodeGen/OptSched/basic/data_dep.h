...
enum LATENCY_PRECISION {
  ...
  LTP_ROUGH,
  // unity: set all latencies to unity, thus ignoring ILP and scheduling only for RP 
  LTP_UNITY
};
...
class DataDepGraph : public DirAcycGraph, public DataDepStruct {
  public:
    ...
    FUNC_RESULT WriteToFile(FILE* file,
                            FUNC_RESULT rslt,
                            InstCount imprvmnt,
                            long number);
    ...
    void GetFileSchedBounds(InstCount& lwrBound,
                            InstCount& uprBound) const;

    InstCount GetFileSchedLngth() {
      return fileSchedLngth_;
    }
    ...
    void GetFinalBounds(InstCount& lwrBound,
                               InstCount& uprBound);

    //Set the final lower and upper bounds
    void SetFinalBounds(InstCount lwrBound,
                               InstCount uprBound);
    ...
    bool UseFileBounds();
    void PrintLwrBounds(DIRECTION dir,
                        std::ostream& out,
    ...
    InstCount GetRltvCrtclPath(SchedInstruction* ref,
                                 SchedInstruction* inst,
                                 DIRECTION dir);
...
//  InstCount totLtncySum_;
    ...
    FUNC_RESULT ReadInstName_(SpecsBuffer* buf,
                              int i,
                              char* instName,
                              char* prevInstName,
                              char* opCode,
    ...
    SchedInstruction* CreateNode_(InstCount instNum,
...
class DataDepSubGraph : public DataDepStruct {
  protected:
    ...
    void CmputCrtclPaths_();
    void FindFrstCycleRange_(InstCount& minFrstCycle,
                             InstCount& maxFrstCycle);
    InstCount GetRealInstCnt_();

    bool TightnDynmcLwrBound_(InstCount frstCycle,
                              InstCount minLastCycle,
                              InstCount maxLastCycle,
                              InstCount trgtLwrBound,
                              InstCount& dynmcLwrBound);
    bool SetDynmcLwrBounds_(InstCount frstCycle,
                            InstCount lastCycle,
                            InstCount shft,
                            InstCount trgtLwrBound,
                            bool useDistFrmLeaf,
                            bool& trgtFsbl);
    bool SetDynmcFrwrdLwrBounds_(InstCount frstCycle,
                                 InstCount lastCycle,
                                 InstCount shft);
    bool SetDynmcBkwrdLwrBounds_(InstCount lastCycle,
                                 InstCount shft,
                                 bool useDistFrmLeaf);

    bool ChkInstRanges_(InstCount lastCycle);
    bool ChkInstRange_(SchedInstruction* inst,
                       InstCount indx,
                       InstCount lastCycle);
    bool CmputSmplDynmcLwrBound_(InstCount& dynmcLwrBound,
                                 InstCount trgtLwrBound,
                                 bool& trgtFsbl);
    ...
    void PropagateBkwrdLwrBounds_(InstCount frmIndx, InstCount toIndx,
                                  InstCount LwrBounds[], bool reset);
    void TightnLwrBound_(DIRECTION dir, InstCount indx,
                         InstCount lwrBounds[]);
    void AllocSttcData_();
...
//  void CmputExtrnlLtncs_(InstCount rejoinCycle);
    InstCount CmputExtrnlLtncs_(InstCount rejoinCycle,
                                  SchedInstruction* inst);

    InstCount CmputExtrnlLtncy_(SchedInstruction* pred,
    ...
    void AllocRlxdSchdulr_(LB_ALG lbAlg,
  ...
  public:
    DataDepSubGraph(DataDepGraph* fullGraph, InstCount maxInstCnt, MachineModel* machMdl);
...
class InstSchedule {
  private:
    ...
    void GetCycleAndSlotNums_(InstCount globSlotNum,
                                     InstCount& cycleNum,
                                     InstCount& slotNum);
